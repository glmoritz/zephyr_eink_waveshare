/*
 * LLSS server CA certificate — ISRG Root X1 (root CA for Let's Encrypt E8).
 *
 * The server at eink.tutu.eng.br:9608 presents a Let's Encrypt certificate
 * signed by E8, which chains to ISRG Root X1.
 *
 * To regenerate:
 *   curl -s https://letsencrypt.org/certs/isrgrootx1.der -o /tmp/llss_ca.der
 *   python3 -c "
 *   data = open('/tmp/llss_ca.der','rb').read()
 *   hex_bytes = ', '.join(f'0x{b:02x}' for b in data)
 *   chunks = [hex_bytes[i:i+72] for i in range(0, len(hex_bytes), 72)]
 *   print('static const uint8_t llss_ca_cert[] = {')
 *   for c in chunks: print('    ' + c)
 *   print('};')
 *   print(f'static const size_t llss_ca_cert_len = {len(data)};')
 *   " > src/certs/ca_cert.h
 */
...
/* ISRG Root X1 — DER, 1391 bytes. Valid until 2035-06-04. */
...
static const size_t llss_ca_cert_len = 1391;
